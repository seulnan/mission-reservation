...
public class ReservationValidator {
    ...
    public static void validateReservationTime(Reservation r) {
        if (r.getStartsAt().isAfter(r.getEndsAt())) {
            throw new IllegalArgumentException("시작 날짜가 종료 날짜보다 늦습니다.");
        }
        if (r.getStartsAt().toLocalDate().isBefore(START_DATE) ||
                r.getEndsAt().toLocalDate().isAfter(END_DATE)) {
    ...
    public static void validateReservationOverlap(Reservation newRes, List<Reservation> existing) {
        for (Reservation r : existing) {
            boolean overlaps = newRes.getStartsAt().isBefore(r.getEndsAt()) &&
                    newRes.getEndsAt().isAfter(r.getStartsAt());
            if (overlaps) {
